...
class TimelineViewModel(private val mealDao: MealDao) : ViewModel() {

    val timelineState: StateFlow<List<DayPlan>> = mealDao.getAllMeals()
        .map { mealsWithDishes ->
    ...
    private fun generateTimeline(mealsWithDishes: List<MealWithDishes>): List<DayPlan> {
        ...
        // Group actual data by ISO date string
        ...
        genCalendar.add(Calendar.DAY_OF_YEAR, -10) // Start from 10 days ago

        for (i in 0..20) { // 10 days past + today + 10 days future = 21 days
...
class TimelineViewModelFactory(private val mealDao: MealDao) : ViewModelProvider.Factory {
    override fun <T : ViewModel> create(modelClass: Class<T>): T {
        if (modelClass.isAssignableFrom(TimelineViewModel::class.java)) {
            @Suppress("UNCHECKED_CAST")
            return TimelineViewModel(mealDao) as T
        }
